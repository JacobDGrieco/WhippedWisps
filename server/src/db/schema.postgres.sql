...
ALTER TABLE photos
ADD COLUMN IF NOT EXISTS image_type TEXT NOT NULL DEFAULT 'final';
...
BEGIN
	IF NOT EXISTS (
		SELECT 1
		FROM pg_constraint
		WHERE conname = 'photos_image_type_check'
	) THEN
		ALTER TABLE photos
		ADD CONSTRAINT photos_image_type_check CHECK (image_type IN ('reference', 'final'));
	END IF;
END $$;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_tags_lower_name_unique ON tags(lower(name));
